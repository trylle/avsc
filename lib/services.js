...
Client.prototype._createMessageHandler$ = function (msg) {
  ...
  var body = function() {
    var args = arguments;
    var opts = args[names.length];
    var cb = args[names.length+1];
    var req = {};

    names.forEach(function (n, idx) { req[n] = args[idx]; });
    //for (var i = 0; i<names.length; ++i) req[names[i]] = args[i];

    return this.emitMessage(msgName, req, opts, cb);
  };

  Object.defineProperty(body, 'name', { value: msgName, writable: false });
...
Server.prototype._createMessageHandler = function (msg) {
  ...
  var body = function (handler) {
    if (handler.length > numArgs) {
      return this.onMessage(name, function (req, cb) {
        var args = fields.map(function (n) { return req[n.name]; });
      ...
      return this.onMessage(name, function (req) {
        var args = fields.map(function (n) { return req[n.name]; });
